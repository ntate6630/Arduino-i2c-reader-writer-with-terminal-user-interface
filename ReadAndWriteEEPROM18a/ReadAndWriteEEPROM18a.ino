...
void loop()
{
    if(doOnce == false)
    {
        ...
        Serial.print("\t\t3. Read from device and Send\n");
        Serial.print("\t\t4. Receive and Write to device\n");
        Serial.print("\t\t5. Edit EEPROM\n");
...
byte ReadEEPROMwriteHexFile(bool exitLoop)
{
    if(byteCounter > 0x10)
        byteCounter = 0x10;
    ...
    { 
        if(byteCounter > 0)
        {
            lineOfData[state] = readEEPROM(currentAddress, deviceAddress);      // Read i2c EEPROM.
            byteCounter--;
            currentAddress++;
            rawHex = true;
            return exitLoop = false;
            ...
            {          
                ...
                {  
                    ...
                    {   
                        ...
                        if(exitLoop == false)
                        {
                            exitLoop = ReadEEPROMwriteHexFile(exitLoop);
                            if(rawHex == false)
