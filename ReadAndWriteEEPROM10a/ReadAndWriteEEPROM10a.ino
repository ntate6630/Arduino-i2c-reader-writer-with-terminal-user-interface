...
int xmodemReceive(void)
{
    ...
    unsigned char rxbuff[133], packetNumber = 1, trychar = 'C';
    byte temp, destination[136];
    delay(12000);
    while(1) 
    {
        for(retry = 0; retry < 16; ++retry) 
...
start_reception:   
        ...
        if(rxbuff[1] == (unsigned char)(~rxbuff[2]) && (rxbuff[1] == packetNumber || rxbuff[1] == (unsigned char)packetNumber - 1) && check(crc, &rxbuff[3], bufferSize) == 0)
        {
           ...
           //     {
                    if(destination[0] == ':')
                    {
                        lineOfData[1] = char2hex(destination[1]) << 4 & 0xF0;
                        temp = char2hex(destination[2]) & 0x0F;
                        lineOfData[1] = lineOfData[1] | temp;
                        Serial1.print("\n byteCount = ");
                        Serial1.print(lineOfData[1], HEX);
                        Serial1.print(" ");
                        for(i = 0; i < 4; i++)
                        { 
                            startAddress = startAddress << 4 | char2hex(destination[3 + i]);
                        }
                        Serial1.print("startAddress = ");
                        Serial1.print(startAddress, HEX);
                        Serial1.print(" ");
                        lineOfData[2] = startAddress >> 8; 
                        lineOfData[3] = startAddress & 0x00FF;
                        Serial1.print(lineOfData[2], HEX);
                        Serial1.print(lineOfData[3], HEX);
                        Serial1.print(" ");
                        lineOfData[4] = char2hex(destination[7]) << 4 & 0xF0;
                        temp = char2hex(destination[8]) & 0x0F;
                        lineOfData[4] = lineOfData[4] | temp;
                        Serial1.print("recordType = ");
                        Serial1.print(lineOfData[4], HEX);
                        Serial1.print(" ");
                        if(lineOfData[4] == 0x00)
                        {
                            counter = 0;
                            for(i = 0; i < lineOfData[1] + 1; i++)      // Add on one more for the chechsum byte.
                            {
                                lineOfData[5 + i] = char2hex(destination[9 + counter]) << 4 & 0xF0;
                                temp = char2hex(destination[10 + counter]) & 0x0F;
                                lineOfData[5 + i] = lineOfData[5 + i] | temp;
                                counter = counter + 2;
                            ...
                            if(verifyData() == 1)
                            {
                                return -6;    // Intel HEX record checksum fail.
                            }
                            else
                                Serial1.print("Checksum OK ");
                        }
                        if(lineOfData[4] == 0x01)
                        {
