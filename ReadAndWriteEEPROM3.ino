...
void hexArrayToCharArray()
{
    byte byteCount, n, i = 0;
    lineOfChar[0] = lineOfData[0];                  // The ':' is already a char so no need for conversion.
    //Serial.print(lineOfChar[0]);
    byteCount = lineOfData[1];                      // Update the byte count from the lineOfData array.
    lineOfChar[1] = hex2char(lineOfData[1] >> 4);    // Convert the byte count value to ASCII and save to char array..
    lineOfChar[2] = hex2char(lineOfData[1]);
    n = 3;
    for(i = 2; i <= byteCount + 6; i++)
    {
        lineOfChar[n] = hex2char(lineOfData[i] >> 4);
        lineOfChar[n + 1] = hex2char(lineOfData[i]);   
      //  Serial.print("\n i = ");
      //  Serial.print(i, HEX);
      //  Serial.print("\n n = ");
      //  Serial.print(n, HEX);
      //  Serial.print("\n"); 
      //  Serial.write(lineOfChar[n]);
      //  Serial.write(lineOfChar[n + 1]); 
        n = n + 2; 
    } 
    
    //Serial.print(lineOfChar[i], HEX);
    //Serial.print(lineOfChar[i + 1], HEX);
}
...
int xmodemTransmit()
{
    ...
    currentAddress = startAddress;
    testDataForHexFile();      // Temporary data.
    while(1) 
    {
